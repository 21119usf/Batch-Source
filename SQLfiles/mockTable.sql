...
CREATE OR REPLACE PROCEDURE GET_ALL_PENDING_ACCOUNTS
(PENDING_ACCOUNTS OUT SYS_REFCURSOR)
AS 
BEGIN 
  OPEN PENDING_ACCOUNTS FOR
    SELECT USER_ID, FIRST_NAME, LAST_NAME, STREET_ADDRESS, EMAIL, PHONE_NUM, APPROVED, IS_ADMIN
    FROM   USERS_TABLE
    WHERE   APPROVED = 0;
END;
/

CREATE OR REPLACE PROCEDURE GET_ALL_USER_ACCOUNTS
(USER_ACCOUNTS OUT SYS_REFCURSOR)
AS 
BEGIN 
  OPEN USER_ACCOUNTS FOR
    SELECT USER_ID, FIRST_NAME, LAST_NAME, STREET_ADDRESS, EMAIL, PHONE_NUM, APPROVED, IS_ADMIN
    FROM   USERS_TABLE
    WHERE   IS_ADMIN = 0 AND APPROVED = 1;
END;
/

CREATE OR REPLACE PROCEDURE GET_ALL_ADMIN_ACCOUNTS
(USER_ACCOUNTS OUT SYS_REFCURSOR)
AS 
BEGIN 
  OPEN USER_ACCOUNTS FOR
    SELECT USER_ID, FIRST_NAME, LAST_NAME, STREET_ADDRESS, EMAIL, PHONE_NUM, APPROVED, IS_ADMIN
    FROM   USERS_TABLE
    WHERE   IS_ADMIN = 1 AND APPROVED = 1;
END;
/
...
INSERT INTO USERS_TABLE VALUES(31,'luis','VENEGAS','SERENITY','MARKA','9999',0,1);
